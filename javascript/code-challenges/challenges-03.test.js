...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  return arr.filter((nums) => !forbiddenValues.includes(nums));
};
...
// };
const getBaseStatGreaterThan = (arr, minBaseStat) => arr.filter(num => minBaseStat < num.baseStat);
...
//   // Solution code here...
//   var test = arr.filter(num => minBaseStat < num.baseStat);
//   // console.log(test.stat.name);
//   let test2 = test.map(item => {
//     return (item.stat.name);
//   });
//   return test2;
// };
const getStatName = (arr, minBaseStat) => arr.filter(num => minBaseStat < num.baseStat).map(item => item.stat.name);
// CHANCE ^
