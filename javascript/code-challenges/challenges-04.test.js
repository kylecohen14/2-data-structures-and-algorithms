...
const updateAnimal = (arr, callback) => {
  // Solution code here...
  let newArr = arr.map( string => callback(string))
  return newArr;
};
...
const sortByPrice = (arr) => {
  // Solution code here...
  return arr.sort ( (firstItem, secondItem) => firstItem.price - secondItem.price);
};
...
const sortNumbersByLength = (arr) => {
  ...
  return newArr.sort( (a,b) => a.length < b.length ? -1 : 1).map(number => parseFloat(number));
};
